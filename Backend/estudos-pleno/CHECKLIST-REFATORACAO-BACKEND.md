...
- ⬜ Repository pattern (isolar as chamadas `prismaClient.x` do Service) — cogitado, ainda não implementado em nenhum módulo.
- ⬜ Replicar esse padrão pros outros ~100 controllers, módulo por módulo (decidido: um de cada vez, com check-in antes de seguir pro próximo).
...
- ⬜ Replicar em `ListTecnicoController.ts` (mesmo problema: lista que muda raramente, buscada do zero toda vez).
...
- ✅ 50 testes unitários passando (Vitest, Prisma mockado) — cobrindo auth, RBAC/CASL, Create/Update de OrdemdeServico, a infra de validação/erro, e agora o cache-aside da listagem.
...
1. **Reorganização de estrutura** — respondido e piloto aplicado (item 1).
...
1. Continuar o rollout de Zod + arquitetura Controller/Service pros outros módulos (item 1 e 3 andam juntos).
