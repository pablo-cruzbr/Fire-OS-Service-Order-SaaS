...
## Estado atual, resumo (atualizado 22/09/2026, depois do rollout de Cliente/Setor/Tecnico)
...
🟡 **30 módulos cobertos, rollout pendente nos outros ~70:** item 1 (Repository pattern em OrdemdeServico, `user`, os 8 módulos de `controles_forms`, Equipamento, InformacoesSetor, os 13 de lookup via o Repository genérico, InstituicaoUnidade, Eventos + Foto de OS, e agora Cliente + Setor + Tecnico), item 3 (Zod nesses mesmos 30). **OrdemdeServico é o primeiro módulo 100% completo** (22/09) — não só o piloto Create/Update, mas List/Get/controle de tempo/assinatura também, com 2 arquivos de código morto achados e apagados no caminho (`CreateAssinaturaController.ts`, `GetAssinaturaController.ts`, nunca ligados a rota nenhuma). ✅ **Item 4 (`try/catch` antigo) fechado por completo** — os 2 últimos arquivos (`Eventos/EventosControllers.ts`, `fotoController.ts`) modernizados no mesmo dia; zero arquivos com o padrão antigo restam no projeto.
...
- ⬜ Replicar pro restante de `status_categorias` e pro resto do projeto (~70 controllers).
