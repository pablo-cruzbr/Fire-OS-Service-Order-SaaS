...
## Estado atual, resumo (atualizado 22/09/2026, depois do bug de Estabilizadores e do grupo Detail)
...
🟡 **31 módulos cobertos, rollout pendente nos outros ~69:** item 1 (Repository pattern em OrdemdeServico, `user`, os 8 módulos de `controles_forms`, Equipamento, InformacoesSetor, os 13 de lookup via o Repository genérico, InstituicaoUnidade, Eventos + Foto de OS, Cliente + Setor + Tecnico, e agora EquipamentoEstabilizador), item 3 (Zod nesses mesmos 31). **OrdemdeServico é o primeiro módulo 100% completo** (22/09) — não só o piloto Create/Update, mas List/Get/controle de tempo/assinatura também, com 2 arquivos de código morto achados e apagados no caminho (`CreateAssinaturaController.ts`, `GetAssinaturaController.ts`, nunca ligados a rota nenhuma). ✅ **Item 4 (`try/catch` antigo) fechado por completo** — os 2 últimos arquivos (`Eventos/EventosControllers.ts`, `fotoController.ts`) modernizados no mesmo dia; zero arquivos com o padrão antigo restam no projeto. **Os 7 endpoints `Detail*` de `controles_forms`** (skipados quando os módulos-pai foram migrados) fechados também (22/09) — não contam como módulo novo, são parte dos 8 já contados.
...
- 🟡 Piloto aplicado também em **`user`**, os **8 módulos de `controles_forms`**, **Equipamento + InformacoesSetor**, **as 13 tabelas de lookup de `status_categorias`**, **InstituicaoUnidade (Update)** (15/09), **Eventos + Foto de OS**, e agora **Cliente + Setor + Tecnico** (22/09) — Controller virou camada fina (só fala com Express), Service só recebe dado e devolve resultado — sem `req`/`res` dentro da lógica de negócio.
...
- ⬜ Replicar pro restante de `status_categorias` e pro resto do projeto (~69 controllers).
