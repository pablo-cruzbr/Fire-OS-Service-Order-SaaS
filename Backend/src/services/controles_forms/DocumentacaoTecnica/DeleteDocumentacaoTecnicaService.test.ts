...
import { DeleteDocumentacaoTecnicaService } from './DeleteDocumentacaoTecnicaService'
...
describe('DeleteDocumentacaoTecnicaService', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('deleta pelo id e devolve mensagem de sucesso', async () => {
    ...
    const service = new DeleteDocumentacaoTecnicaService(repository)
    const resultado = await service.execute('reg-1')

    expect(repository.delete).toHaveBeenCalledWith('reg-1')
    expect(resultado.message).toBe('Controle de Documentação técnica deletado com sucesso.')
  })
})
