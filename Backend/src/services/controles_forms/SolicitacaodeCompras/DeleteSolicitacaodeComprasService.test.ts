...
import { DeleteSolicitacaodeComprasService } from './DeleteSolicitacaodeComprasService'
...
describe('DeleteSolicitacaodeComprasService', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('deleta pelo id e devolve mensagem de sucesso', async () => {
    ...
    const service = new DeleteSolicitacaodeComprasService(repository)
    const resultado = await service.execute('reg-1')

    expect(repository.delete).toHaveBeenCalledWith('reg-1')
    expect(resultado.message).toBe('Controle de Solicitacao de Compras deletado com sucesso.')
  })
})
