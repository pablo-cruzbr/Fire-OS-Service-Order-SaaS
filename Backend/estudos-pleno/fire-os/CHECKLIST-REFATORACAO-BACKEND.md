...
**O que falta agora, em uma frase:** o rollout de Zod/Repository (item 1/3) está fechado — o que resta é ampliar o item 7 (E2E pra fluxos ainda fora do escopo: upload/fila, eventos, Deletes de `controles_forms`), e a mudança de estratégia de testes já combinada mas ainda não executada (Redis via TestContainers + testes de integração real pros 21 Repositories, hoje só provados com Prisma mockado).
