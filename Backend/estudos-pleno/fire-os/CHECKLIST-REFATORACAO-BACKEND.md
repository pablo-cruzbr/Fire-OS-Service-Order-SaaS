...
🟡 **27 módulos cobertos, rollout pendente nos outros ~73:** item 1 (Repository pattern em OrdemdeServico, `user`, os 8 módulos de `controles_forms`, Equipamento, InformacoesSetor, os 13 de lookup via o Repository genérico, InstituicaoUnidade, e agora Eventos + Foto de OS), item 3 (Zod nesses mesmos 27). **OrdemdeServico é o primeiro módulo 100% completo** (22/09) — não só o piloto Create/Update, mas List/Get/controle de tempo/assinatura também, com 2 arquivos de código morto achados e apagados no caminho (`CreateAssinaturaController.ts`, `GetAssinaturaController.ts`, nunca ligados a rota nenhuma). ✅ **Item 4 (`try/catch` antigo) fechado por completo** — os 2 últimos arquivos (`Eventos/EventosControllers.ts`, `fotoController.ts`) modernizados no mesmo dia; zero arquivos com o padrão antigo restam no projeto.
...
- 🟡 Piloto aplicado também em **`user`**, os **8 módulos de `controles_forms`**, **Equipamento + InformacoesSetor**, **as 13 tabelas de lookup de `status_categorias`**, **InstituicaoUnidade (Update)** (15/09), e agora **Eventos + Foto de OS** (22/09) — Controller virou camada fina (só fala com Express), Service só recebe dado e devolve resultado — sem `req`/`res` dentro da lógica de negócio.
