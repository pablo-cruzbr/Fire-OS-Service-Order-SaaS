...
## Estado atual, resumo (atualizado 22/09/2026, depois de fechar tudo em torno de OrdemdeServico)
...
🟡 **32 módulos cobertos, rollout pendente nos outros ~68:** item 1 (Repository pattern em OrdemdeServico, `user`, os 8 módulos de `controles_forms`, Equipamento, InformacoesSetor, os 13 de lookup via o Repository genérico, InstituicaoUnidade, Eventos + Foto de OS, Cliente + Setor + Tecnico, EquipamentoEstabilizador, e agora AtividadePadrao), item 3 (Zod nesses mesmos 32). ✅ **OrdemdeServico e tudo que a rodeia diretamente está 100% completo** (22/09) — Create/Update/List/Get/controle de tempo/assinatura (piloto original + fechado), mais os 3 endpoints de leitura que sobravam (`GET /ordens/exportar`, `GET /ordens/relatorio-secretaria`, `GET /listatividade`), mais os 7 `Detail*` dos 8 módulos de `controles_forms` que ficam ao redor de uma OS. 2 arquivos de código morto achados e apagados no caminho (`CreateAssinaturaController.ts`, `GetAssinaturaController.ts`, nunca ligados a rota nenhuma). ✅ **Item 4 (`try/catch` antigo) fechado por completo** — os 2 últimos arquivos (`Eventos/EventosControllers.ts`, `fotoController.ts`) modernizados no mesmo dia; zero arquivos com o padrão antigo restam no projeto.
...
- ⬜ Replicar pro restante de `status_categorias` e pro resto do projeto (~66 controllers).
